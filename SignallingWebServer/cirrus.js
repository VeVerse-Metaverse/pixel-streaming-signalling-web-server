...
if(config.EnableWebserver) {
	...
	const homepageHandler = function (req, res) {
		let homepageFile = (typeof config.HomepageFile != 'undefined' && config.HomepageFile !== '') ? config.HomepageFile.toString() : defaultConfig.HomepageFile;
	...
	app.get('/', isAuthenticated('/login'), homepageHandler);

	app.get(`/${INSTANCE_ID}`, isAuthenticated('/login'), homepageHandler);
}
...
function closeStreamingSession(sessionId) {
	axios.delete(`http://127.0.0.1:8080/session`).then((response) => {
		logger(`Pixel Streaming session closed: ${response}`);
	}, (error) => {
		logger(`Error closing Pixel Streaming session: ${error}`);
	});
}

checkPlayerCountTotal = 0;
const healthCheckIntervalId = setInterval(() => {
	logger(`player count: ${playerCount}`);
	logger(`checkPlayerCountTotal: ${checkPlayerCountTotal}`);
	if (playerCount === 0 && checkPlayerCountTotal <= 20) {
		axios.get(`http://127.0.0.1:8080/healthcheck?totalCheck=${checkPlayerCountTotal}`).then((response) => {
			logger(`healthcheck: ${response.data.sessionStatus}`);
			const { data: { sessionStatus } } = response;

			if (sessionStatus === 'running') {
				checkPlayerCountTotal++;
			}
		}, (error) => {
